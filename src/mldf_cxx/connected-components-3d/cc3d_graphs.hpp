...
inline void compute_neighborhood_all(
	int* neighborhood,
	const int x, const int y, const int z,
	...
	// 6-hood
	neighborhood[3] = 1 * (x > 0); // -x
	neighborhood[4] = sx * (y > 0); // -y
	neighborhood[5] = sxy * (z > 0); // -z
	...
	// xy diagonals
	neighborhood[12] = (connectivity > 6) * (x > 0 && y > 0) * (-1 + sx); // up-left
	neighborhood[13] = (connectivity > 6) * (x < sx - 1 && y > 0) * (1 + sx); // up-right

	// yz diagonals
	neighborhood[14] = (connectivity > 6) * (y > 0 && z > 0) * (-sx + sxy); // down-left
	neighborhood[15] = (connectivity > 6) * (y < sy - 1 && z > 0) * (sx + sxy); // down-right

	// xz diagonals
	neighborhood[16] = (connectivity > 6) * (x > 0 && z > 0) * (-1 + sxy); // down-left
	neighborhood[17] = (connectivity > 6) * (x < sx - 1 && z > 0) * (1 + sxy); // down-right
	...
	// Now the four corners of the bottom plane
	neighborhood[22] = (connectivity > 18) * (x > 0 && y > 0 && z > 0) * (-1 - sx + sxy);
	neighborhood[23] = (connectivity > 18) * (x < sx - 1 && y > 0 && z > 0) * (1 - sx + sxy);
	neighborhood[24] = (connectivity > 18) * (x > 0 && y < sy - 1 && z > 0) * (-1 + sx + sxy);
	neighborhood[25] = (connectivity > 18) * (x < sx - 1 && y < sy - 1 && z > 0) * (1 + sx + sxy);
}
