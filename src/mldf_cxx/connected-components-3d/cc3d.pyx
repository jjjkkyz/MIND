...
cdef extern from "cc3d.hpp" namespace "cc3d":
  cdef U* connected_components_cross_imp[T,U](
    T* in_labels,
    int8_t* wn_sign,
    int64_t sx, int64_t sy, int64_t sz,
    int64_t max_labels ,size_t &N,
    U * out_labels,
  ) except +
...
cdef extern from "cc3d_graphs.hpp" namespace "cc3d":
  ...
  cdef cpp_pair[unordered_map[cpp_pair[T,T], float, pair_hash],unordered_map[cpp_pair[T,T], float, pair_hash]] extract_region_graph_background[T](
    T* labels,
    uint8_t * mask,
    int8_t* wn_sign,
    int64_t sx, int64_t sy, int64_t sz,
  ...
  cdef T* erode_ml_c[T](
    T* in_labels,
    int64_t sx, int64_t sy, int64_t sz,
    int64_t connectivity, int64_t it,
    T * out_labels,
  ) except +

  cdef T* dilate_ml_c[T](
    T* in_labels,
    uint8_t* mask,
    int8_t * wn_sign,
    int64_t sx, int64_t sy, int64_t sz,
...
def erode_ml(
  ...
  if dims not in (3,):
    raise DimensionError("Only 3D arrays supported. Got: " + str(dims))

  if dims == 3 and connectivity not in (6, 18, 26):
  ...
  cdef cnp.ndarray[uint8_t, ndim=1] out_labels8 = np.array([], dtype=np.uint8)
  ...
  def output_factory(voxels, out_dtype):
    return np.zeros( (voxels,), dtype=out_dtype, order='F' )

  if out_dtype == np.uint16:
    out_labels16 = output_factory(voxels, out_dtype)
    out_labels = out_labels16
  elif out_dtype == np.uint32:
    out_labels32 = output_factory(voxels, out_dtype)
    out_labels = out_labels32
  elif out_dtype == np.uint8:
    out_labels8 = output_factory(voxels, out_dtype)
    out_labels = out_labels8
  elif out_dtype == np.uint64:
    out_labels64 = output_factory(voxels, out_dtype)
  ...
  try:
    ...
    if out_dtype == np.uint8:
      ...
      erode_ml_c[uint8_t](
        &arr_memview8u[0,0,0],
        sx, sy, sz, connectivity, it,
        <uint8_t*> &out_labels8[0],
      )
...
def dilate_ml(
  ...
  cdef uint8_t[:,:,:] mask_view
  cdef int8_t[:,:,:] arr_wn_sign
  cdef uint8_t[:,:,:] arr_memview8u
  cdef uint16_t[:,:,:] arr_memview16u
  cdef uint32_t[:,:,:] arr_memview32u
  cdef uint64_t[:,:,:] arr_memview64u
  ...
  try:
    ...
    arr_wn_sign = wn_sign.view(np.int8)
    if out_dtype == np.uint16:
      arr_memview16u = data.view(np.uint16)
      dilate_ml_c[uint16_t](
        &arr_memview16u[0,0,0],
        &mask_view[0,0,0],
        &arr_wn_sign[0, 0, 0],
        sx, sy, sz, connectivity, it,
...
@cython.binding(True)
def connected_components_cross(
  data:np.ndarray,
  wn_sign:np.ndarray,
  int64_t max_labels=-1,
  out_dtype:Optional[Any] = None,
  ...
  try:
    ...
    arr_memview8u = data.view(np.uint8)
    if out_dtype == np.uint8:
      connected_components_cross_imp[uint8_t,uint8_t](
        &arr_memview8u[0,0,0],
        &arr_wn_sign[0,0,0],
        sx, sy, sz, max_labels, N,
        <uint8_t*> &out_labels8[0],
      )
    elif out_dtype == np.uint16:
      connected_components_cross_imp[uint8_t,uint16_t](
        &arr_memview8u[0,0,0],
        &arr_wn_sign[0,0,0],
        sx, sy, sz, max_labels, N,
        <uint16_t*> &out_labels16[0],
      ...
      connected_components_cross_imp[uint8_t,uint32_t](
        &arr_memview8u[0,0,0],
        &arr_wn_sign[0,0,0],
        sx, sy, sz, max_labels, N,
        <uint32_t*> &out_labels32[0],
      ...
      connected_components_cross_imp[uint8_t,uint64_t](
        &arr_memview8u[0,0,0],
        &arr_wn_sign[0,0,0],
        sx, sy, sz, max_labels, N,
        <uint64_t*> &out_labels64[0],
...
def contacts_background(
  ...
  return _contacts_background(labels,inner_mask, wn_sign, connectivity, surface_area, anisotropy)


def _contacts_background(
  cnp.ndarray[INTEGER, ndim=3, cast=True] labels,
  cnp.ndarray[uint8_t, ndim=3, cast=True] inner_mask,
  cnp.ndarray[int8_t, ndim=3, cast=True] wn_sign,
  int connectivity=26,
  surface_area=True,
  anisotropy=(1,1,1),
):
  ...
  labels = np.asfortranarray(labels)
  wn_sign = np.asfortranarray(wn_sign)
  inner_mask = np.asfortranarray(inner_mask)
  cdef cpp_pair[unordered_map[cpp_pair[INTEGER,INTEGER], float, pair_hash],
                unordered_map[cpp_pair[INTEGER,INTEGER], float, pair_hash]] res = extract_region_graph_background(
    <INTEGER*>&labels[0,0,0],
    <uint8_t *> &inner_mask[0, 0, 0],
    <int8_t*>&wn_sign[0, 0, 0],
    labels.shape[0], labels.shape[1], labels.shape[2],
    anisotropy[0], anisotropy[1], anisotropy[2],
    connectivity, surface_area
  )
  ...
  for pair in res.first:
    output[(pair.first.first, pair.first.second)] = pair.second
  output_2 = {}
  for pair in res.second:
    output_2[(pair.first.first, pair.first.second)] = pair.second
