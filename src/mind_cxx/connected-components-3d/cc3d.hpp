  ...
  if (N < static_cast<size_t>(num_labels)) {
    ...
    for (int64_t row = 0; row < sy * sz; row++) {
      ...
      for (int64_t loc = sx * row + xstart; loc < sx * row + xend; loc++) {
          if (out_labels[loc] == 0 && renumber[out_labels[loc]] != 0) {
              std::cout << "???" << std::endl;
        }
...
template <typename T, typename OUT = uint32_t>
OUT* connected_components_cross_imp(
    T* in_labels,
    int8_t* wn,
    const int64_t sx, const int64_t sy, const int64_t sz,
    size_t max_labels,
  ...
  for (int64_t z = 0; z < sz; z++) {
    for (int64_t y = 0; y < sy; y++, row++) {
      ...
      for (int64_t x = xstart; x < xend; x++) {
        ...
        if (x > 0 && (wn[loc]^wn[loc + M]) >= 0 && out_labels[loc + M] != 0) {
          out_labels[loc] = out_labels[loc + M];

          if (y > 0 && (wn[loc] ^ wn[loc + K]) >= 0 && out_labels[loc + K] != 0) {
            equivalences.unify(out_labels[loc], out_labels[loc + K]);
            if (z > 0 && (wn[loc] ^ wn[loc + E]) >= 0 && out_labels[loc + E] != 0) {
              equivalences.unify(out_labels[loc], out_labels[loc + E]);
            }
          }
          else if (z > 0 && (wn[loc] ^ wn[loc + E]) >= 0 && out_labels[loc + E] != 0) {
            equivalences.unify(out_labels[loc], out_labels[loc + E]);
          }
        }
        else if (y > 0 && (wn[loc] ^ wn[loc + K]) >= 0 && out_labels[loc + K] != 0) {
          out_labels[loc] = out_labels[loc + K];
  ...
  for (int64_t z = 0; z < sz; z++) {
    for (int64_t y = 0; y < sy; y++, row++) {
      ...
      for (int64_t x = xstart; x < xend; x++) {
        ...
        const T cur = in_labels[loc];
        if (cross_x[loc] == 1){
          //std::cout<< "hit_x" << std::endl;
          out_labels[loc] = 100;
        }
        if (cross_y[loc] == 1){
          // std::cout<< "hit_y" << std::endl;
          out_labels[loc] = 200;
        }
        if (cross_z[loc] == 1){
          //std::cout<< "hit_z" << std::endl;
          out_labels[loc] = 300;
        }
      }
    }
