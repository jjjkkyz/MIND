...
template <typename T, typename OUT = uint32_t>
OUT* connected_components_cross_c(
    T* in_labels,
    uint8_t* cross_x,
    uint8_t* cross_y,
    uint8_t* cross_z,
  ...
  // for performance, shouldn't be "more correct"
  if (delta == 0 && connectivity==6) {
    return connected_components_cross_imp<T,OUT>(
      in_labels, cross_x, cross_y, cross_z, sx, sy, sz,
      max_labels,
      out_labels, N, periodic_boundary
    );
  }
  else {
    throw std::runtime_error("Only 3d 6 connectivities 0 delta are supported.");
  }
}
