    ...
    for (int z = 0; z < sz; z++) {
        for (int y = 0; y < sy; y++) {
            for (int x = 0; x < sx; x++) {
                int loc = x + sx * y + sxy * z;
                int fake = labels[loc];
                // 记录所有在erode and relabel中被腐蚀掉的voxel
                if (fake != 0)
                {
                    if (relabels[loc] == 0)
                    {
                    ...
                    //我们也需要与被腐蚀的voxel邻接的voxel加入grid expansion
                    else {
                        compute_neighborhood_all(neighborhood, x, y, z, sx, sy, sz, 6);
                        for (int i = 0; i < 6; i++) {
                            int neighboridx = loc + neighborhood[i];
                            //对于边界voxel，neighborhood[i]为0，而unorient_region[loc]此处必为0，所以不需额外考虑
                            if (relabels[neighboridx] != 0) {
                                //找到了周围的voxel，即使当前voxel在relabel中有label，我们也要把它加入到grid expansion中
                                idx_map[loc] = nPix;
                                inv_idx_map.push_back(loc);
                                nPix++;
